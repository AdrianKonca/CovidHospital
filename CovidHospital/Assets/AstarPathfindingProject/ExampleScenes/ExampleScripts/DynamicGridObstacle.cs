...
namespace Pathfinding
{
	...
	///     When the object has moved a certain distance (or actually when it's bounding box has changed by a certain amount)
	///     defined by <see cref="updateError" />
    ...
    public class DynamicGridObstacle : GraphModifier
    {
        ...
        private void Update()
        {
            ...
            if (AstarPath.active == null || AstarPath.active.isScanning ||
                Time.realtimeSinceStartup - lastCheckTime < checkTime || !Application.isPlaying) return;
            ...
            if (colliderEnabled)
            {
                ...
                if (minDiff.sqrMagnitude > updateError * updateError ||
                    ...
                    errorFromRotation > updateError || !prevEnabled) // Update the graphs as soon as possible
                    DoUpdateGraphs();
            }
            else
            {
