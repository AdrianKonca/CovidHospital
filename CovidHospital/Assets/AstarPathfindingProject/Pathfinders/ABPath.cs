            ...
            while (CompleteState == PathCompleteState.NotCalculated)
            {
                ...
                if (counter > 500)
                {
                    ...
                    if (DateTime.UtcNow.Ticks >=
                        targetTick) // Return instead of yield'ing, a separate function handles the yield (CalculatePaths)
                        return;
                    counter = 0;
                    ...
                    switch (gridGraph.neighbours)
                    {
                        case NumNeighbours.Four:
                            if (x1 == x2 && Math.Abs(z1 - z2) == 1 ||
                                z1 == z2 && Math.Abs(x1 - x2) ==
                                1) // If 'O' is gridNode2, then gridNode is one of the nodes marked with an 'x'
                                //    x
                                //  x O x
                                //    x
                                wasClose = true;
