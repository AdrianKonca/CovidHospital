...
namespace Pathfinding
{
    ...
    public abstract class NavmeshBase : NavGraph, INavmesh, INavmeshHolder, ITransformedGraph
    {
        ...
        private readonly List<int> batchUpdatedTiles = new List<int>();

        /// <summary>List of nodes that are going to be destroyed as part of a batch update</summary>
        private readonly List<MeshNode> batchNodesToDestroy = new List<MeshNode>();
        ...
        private readonly Dictionary<int, int> nodeRecyclingHashBuffer = new Dictionary<int, int>();
        ...
        private void PrepareNodeRecycling(int x, int z, Int3[] verts, int[] tris, TriangleMeshNode[] recycledNodeBuffer)
        {
            ...
            for (var i = 0; i < nodes.Length; i++)
            {
                ...
                int newNodeIndex;
                if (
                        recycling.TryGetValue(hash,
                            out newNodeIndex)) // Technically we should check for a cyclic permutations of the vertices (e.g node a,b,c could become node b,c,a)
